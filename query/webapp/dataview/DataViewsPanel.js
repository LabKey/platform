                            ...
                            if (id == 'ok') {
                                Ext4.Ajax.request({
                                    ...
                                    failure : LABKEY.Utils.getCallbackWrapper(function(json, response, options) {
                                        Ext4.Msg.alert("Delete", "Deletion Failed: " + LABKEY.Utils.encodeHtml(json.exception));
                                    }, null, true)
                                });
                            }
                        }
    ...
    onEditClick : function(view, record) {
                ...
                if (form.isValid())
                {
                    ...
                    form.submit({
                        ...
                        failure : function(form, action) {
                            ...
                            Ext4.Msg.alert("Error", LABKEY.Utils.encodeHtml(msg));
                        },
                        scope : this
                    });
                }
        ...
        if (this.editInfo[record.data.dataType] && this.editInfo[record.data.dataType].actions['delete']) {
            ...
            buttons.push({
                ...
                handler: function ()
                {
                    var msg = 'Are you sure you want to delete "' + LABKEY.Utils.encodeHtml(record.data.name) + '"';
                    ...
                    {
                        Ext4.MessageBox.alert('Error Deleting Report', 'There was an error deleting the report "' + LABKEY.Utils.encodeHtml(record.data.name) + '"');
                        editWindow.close();
                    };
