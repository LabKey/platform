...
public class SqlController extends SpringActionController
{
    ...
    static final char RS = '\036';     // ASCII Record Separator
    static final char US = '\037';     // ASCII Unit Separator
    static final char BS = '\010';     // ASCII Backspace (ditto marker)
    ...
    public enum Format
    {
        split("text/plain"),  // response that can be parsed using String.split(), configure with 'sep' and 'eol'

        compact("text/plain"),    // same as split, but with ditto markers (cheap way to compress and save space on client)
    ...
    public static class SqlForm
    {
        ...
        {
            return null!=sep ? sep : format==Format.compact ? String.valueOf(US) : "\t";
        }
        ...
        {
            return null!=eol ? eol : format==Format.compact ? String.valueOf(RS) : "\n";
        }
    ...
    public static class SqlExecute
    {
        ...
        private final ContainerUser ctx;

        public SqlExecute(ContainerUser ctx, UserSchema schema, String sql)
        {
            this.ctx = ctx;
            this.schema = schema;
            this.sql = sql;
        ...
        SqlExecute parameters(Map<String,Object> parameters)
        {
            this.parameterMap = new HashMap<>(parameters);
            return this;
        }

        SqlExecute format(Format format, String sep, String eol)
        {
            this.format = format;
            this.sep = sep;
            this.eol = eol;
    ...
    public static class ExecuteAction extends ReadOnlyApiAction<SqlForm>
    {
        ...
        public Object execute(SqlForm form, BindException errors) throws ServletException, SQLException, IOException
        {
            ...
            var schema = DefaultSchema.get(getUser(), getContainer(), schemaKey);
    ...
    public static class TestCase extends Assert
    {
        ...
        public void testExecuteCompact() throws Exception
        {
            ...
            String content = response.getContentAsString();
            String sep = String.valueOf(US);

            String eol = String.valueOf(RS);
            String ditto = String.valueOf(BS);
