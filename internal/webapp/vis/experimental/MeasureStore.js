 ...
 * TODO property support (olap term) or dependant columns
 *
 * CONSIDER should I be caching group objects more aggressively? or letting the caller hang on to them?
 * CONSIDER MultiMeasureStore, wrapper for multiple MeasureStores with different key sets
 *
 * I think some of these array methods may not be supported in IE8?  [].map() ? [].forEach()?
 */

window.DEBUG = window.DEBUG || {};

// NOTE: if we are starting with aggregated data, we may want to
// accumulate the data differently.  We would want to accumulate
// count,sum,sumOfSquares as we reduce
// Maybe, a subclass of Measure?
