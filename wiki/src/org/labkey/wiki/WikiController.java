...
public class WikiController extends SpringActionController
{
    ...
    /*
     * UNDONE: too much duplicated code between InsertAction/UpdateAction
     * UNDONE: and wiki_insert.gm wiki_update.gm
     */

    @ActionNames("insert, showInsert")
    @RequiresPermission(ACL.PERM_INSERT)
    public class InsertAction extends FormViewAction<WikiInsertForm>
    {
        private ActionURL _successUrl = null;
        private ModelAndView _errorView = null;

        public ModelAndView getView(WikiInsertForm form, boolean reshow, BindException errors) throws Exception
        {
            if (null != _errorView)
            {
            ...
            //create new page
            Wiki wiki = new Wiki(getContainer(), form.getName());

            WikiRendererType currentRendererType;
            if (form.getRendererType() == null)
                currentRendererType = WikiService.get().getDefaultWikiRendererType();
            else
                currentRendererType = WikiRendererType.valueOf(form.getRendererType());
            
            boolean useVisualEditor;

            //Per bug 3200, switching source editor re-posts to showUpdate so that changes
            // are not saved.  But, we need to look at the action value and update the visual
            // editor option when it happens
            String nextAction = (String)getProperty("nextAction");
            if (nextAction != null && nextAction.startsWith("toggleVisualEditor"))
            {
                //save user's editor preference
                PropertyManager.PropertyMap properties =
                        PropertyManager.getWritableProperties(getUser().getUserId(),
                                getContainer().getId(), "editorPreference", true);
                useVisualEditor = nextAction.equals("toggleVisualEditorOn");
                properties.put("useVisualEditor", String.valueOf(useVisualEditor));
            ...
            else
            {
                Map properties = PropertyManager.getProperties(getUser().getUserId(),
                        getContainer().getId(), "editorPreference", true);
                useVisualEditor = "true".equalsIgnoreCase((String)properties.get("useVisualEditor"));
            }

            JspView insertView = new JspView("/org/labkey/wiki/view/wikiInsert.jsp");
            //insertView.setTitle("Create new page");
            insertView.addObject("_wiki", wiki);
            insertView.addObject("_errors", errors);
            insertView.addObject("_form", form);
            insertView.addObject("_currentRendererType", currentRendererType);
            insertView.addObject("_useVisualEditor", useVisualEditor);
            insertView.addObject("_reshow", reshow);
            ...
            getPageConfig().setFocus("forms[0].name");
            return new VBox(insertView, helpView);
        }
        

        public boolean handlePost(WikiInsertForm form, BindException errors) throws Exception
        {
            // check if we're just toggling editor (don't treat as post)
            if (form.isReshow())
                return false;
            ...
            else if (wikiname.startsWith("_") && !c.hasPermission(getUser(), ACL.PERM_ADMIN))
                error = "Wiki names starting with underscore are reserved for administrators.";
            else
            {
                Wiki old = WikiManager.getWiki(c, wikiname);
                if (old != null)
                    error = "Page '" + wikiname + "' already exists.";
            }

            if (error != null)
            {
                errors.addError(new ObjectError("main", new String[]{"Error"},new Object[]{error}, error));
                return false;
            }

            Wiki wiki = new Wiki(c, wikiname);
            wiki.setParent(form.getParent());
            ...
            WikiManager.insertWiki(getUser(), c, wiki, wikiversion, files);
            ...
            if (redirect.length() > 0)
                _successUrl = new ActionURL(redirect);
            else
            {
                String nextAction = StringUtils.trimToEmpty(form.getNextAction());
                if (nextAction == null || nextAction.length() == 0 || nextAction.compareToIgnoreCase("false") == 0)
                    nextAction = "page";

                _successUrl = getViewContext().cloneActionURL();
                _successUrl.deleteParameters();
                _successUrl.addParameter("name", wikiname);
                _successUrl.setAction(nextAction);
            }

            // If any of the attachments didn't upload then display an interstitial warning page before
            // redirecting to the success url.
            _errorView = AttachmentService.get().getErrorView(files, _successUrl);

            return (null == _errorView);
        }
        ...
        public NavTree appendNavTrail(NavTree root)
        {
            return new BeginAction().appendNavTrail(root)
                    .addChild("Create new page");
        }
    }
    
    
    @RequiresPermission(ACL.PERM_READ) //will test for update or updateown explicitly below
    public class UpdateAction extends FormViewAction<WikiDataForm>
    {
        Wiki _wiki;
        WikiVersion _wikiversion = null;
        
        public ModelAndView getView(WikiDataForm form, boolean reshow, BindException errors) throws Exception
        {
            ...
            if(null == currentRendererName)
                currentRendererName = _wikiversion.getRendererType();
            WikiRendererType currentRendererType;

            //if we are not reloading page due to error, display latest version.
            if (!reshow)
            {
                //on initial load of page, display latest version data
                form.setTitle(_wikiversion.getTitle());
                form.setBody(_wikiversion.getBody());
                currentRendererType = WikiRendererType.valueOf(_wikiversion.getRendererType());
            }
            else
                currentRendererType = WikiRendererType.valueOf(currentRendererName);

            boolean useVisualEditor;
            boolean explicitUseVisualEditor = false;
            ...
            GroovyView updateView = new GroovyView("/org/labkey/wiki/view/wiki_update.gm");

            // webpart title
            //updateView.setTitle(form.getTitle() == null ? "Update Page \"" + form.getName() + "\"" : form.getTitle());
            updateView.addObject("_wiki", _wiki);
            updateView.addObject("_errors", errors);
            updateView.addObject("_form", form);
            ...
            urlDelete.deleteParameters();
            urlDelete.setAction("deleteAttachment");
            updateView.addObject("_deleteUrl", urlDelete.getLocalURIString());

            getPageConfig().setFocus("forms[0].title");
            return updateView;
        }

        public boolean handlePost(WikiDataForm form, BindException errors) throws Exception
        {
            // check if we're just toggling editor (don't treat as post)
            if (form.isReshow())
                return false;
            ...
            String name = form.getName();
            Wiki wikiUpdate = WikiManager.getWiki(getContainer(), name);
            if (wikiUpdate == null)
                HttpView.throwNotFound("There is no wiki page with this name; if it was a valid page, it may have been deleted by another user");
            ...
            String title = form.getTitle() == null ? name : form.getTitle();
            String currentRendererName = form.getRendererType();

            //only insert new version in case where title, body, or renderer has changed
            if (StringUtils.trimToEmpty(wikiversion.getTitle()).compareTo(title) != 0 ||
                    ...
                    wikiversion.getRendererType().compareTo(currentRendererName) != 0)
            {
                ...
                /*
                if (wikiversion.getRendererType().compareTo(WikiRendererType.RADEOX.toString()) == 0 &&
                        currentRendererName.compareTo(WikiRendererType.HTML.toString()) == 0)
                {
                    //When switching from radeox to HTML, render the wiki to html and use that as the new body
                    // Probably need to fix up links to local wiki pages to match HTML syntax for this
                    //TODO:  write something that can do the above
            ...
            WikiManager.updateWiki(getUser(), wikiUpdate, wikiversion);
            ...
            if(null != files && files.size() > 0)
            {
                try
                {
                    AttachmentService.get().addAttachments(getUser(), wikiUpdate, files);
                }
                catch(AttachmentService.DuplicateFilenameException e)
                {
                    StringBuilder rejectMsg = new StringBuilder("Your changes have been saved, but some attachments had duplicate names: ");
                    String eMsg = e.getMessage().trim();
                    rejectMsg.append(eMsg);
                    if(!eMsg.endsWith("."))
                        rejectMsg.append(".");
                    rejectMsg.append(" Please attach a file with a different name, or click Finished to exit.");
                    errors.reject(ERROR_MSG, rejectMsg.toString());
                    return false;
                }
            }
            ...
            //if finished, display updated page
            String redirect = StringUtils.trimToEmpty(form.getRedirect());
            if ("page".equals(nextAction) && redirect.length() > 0)
                HttpView.throwRedirect(redirect);

            ActionURL nextPage = wikiURL(nextAction);
            nextPage.addParameter("name", wikiUpdate.getName());
            nextPage.addParameter("redirect", redirect);
            HttpView.throwRedirect(nextPage);
            return true;
        }
        ...
        {
            return (new PageAction(_wiki,_wikiversion)).appendNavTrail(root)
                    .addChild("Update",getUrl());
        }

        public ActionURL getUrl()
        {
            return wikiURL("update").addParameter("name",_wikiversion.getName());
        }
    }
