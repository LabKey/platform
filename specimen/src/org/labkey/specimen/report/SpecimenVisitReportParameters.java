...
public abstract class SpecimenVisitReportParameters extends ViewForm
{
    ...
    {
        _cohortFilter = CohortService.get().getFromURL(getViewContext().getContainer(), getViewContext().getUser(), getViewContext().getActionURL());
    }
    ...
    protected Pair<String, HtmlString> getParticipantPicker(String inputName, String selectedParticipantId)
    {
        ...
        if (participantIds.size() <= 200)
        {
            ...
            {
                boolean isSelected = (selectedParticipantId != null && selectedParticipantId.equals(participantId)) ||
                        (selectedParticipantId == null && first);
                first = false;

                builder.addOption(new OptionBuilder()
                    .value(participantId)
                    .label(DemoMode.id(participantId, getContainer(), getUser()))
                    .selected(isSelected)
                    .build());
            }
    ...
    public abstract Class<? extends SpecimenVisitReportAction> getAction();
