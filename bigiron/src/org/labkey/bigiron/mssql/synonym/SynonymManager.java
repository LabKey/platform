/*
 * Copyright (c) 2015-2016 LabKey Corporation. All rights reserved. No portion of this work may be reproduced in
...
{
    private static final Logger LOG = LogManager.getLogger(SynonymManager.class);

    // Cache of synonyms by datasource name. This lets admins force a re-query by clearing caches.
    static final Cache<String, Pair<Map<String, Map<String, Synonym>>, Map<String, Map<String, MultiValuedMap<String, Synonym>>>>> SYNONYM_CACHE = CacheManager.getBlockingStringKeyCache(100, CacheManager.YEAR, "SQL Server synonyms", new SynonymLoader());

    private static final Pair<Map<String, Map<String, Synonym>>, Map<String, Map<String, MultiValuedMap<String, Synonym>>>> NO_SYNONYMS;

    static
    {
        Map<String, Map<String, Synonym>> emptyMap = Collections.emptyMap();
        Map<String, Map<String, MultiValuedMap<String, Synonym>>> emptyReverseMap = Collections.emptyMap();
    ...
    private static @NotNull Map<String, MultiValuedMap<String, Synonym>> getReverseMap(DbScope synonymScope, DbScope targetScope)
    {
    ...
    static @NotNull Map<String, Synonym> getSynonymMap(DbScope scope, String schemaName)
    {
        ...
        return null == synonymMap ? Collections.emptyMap() : synonymMap;
    }
    ...
    static @Nullable Pair<DbScope, Synonym> getSynonym(DbScope scope, String schemaName, String requestedTableName)
    {
        ...
        LOG.error("Could not access metadata for " + synonym + ". No " + AppProps.getInstance().getWebappConfigurationFilename() + " datasource is defined for the target database.");
    ...
    private static class SynonymLoader implements CacheLoader<String, Pair<Map<String, Map<String, Synonym>>, Map<String, Map<String, MultiValuedMap<String, Synonym>>>>>
    {
        ...
        public Pair<Map<String, Map<String, Synonym>>, Map<String, Map<String, MultiValuedMap<String, Synonym>>>> load(String key, @Nullable Object argument)
        {
            ...
            new MetadataSqlSelector(scope, "SELECT DISTINCT COALESCE(PARSENAME(base_object_name, 3), DB_NAME(DB_ID())) AS TargetDatabase FROM sys.synonyms").forEach(String.class, dbName -> {
            ...
            final Map<String, Map<String, MultiValuedMap<String, Synonym>>> reverseMap = new HashMap<>();

            new MetadataSqlSelector(scope, sql).forEach(Synonym.class, synonym -> {
                ...
                Map<String, MultiValuedMap<String, Synonym>> schemaMap = reverseMap.get(synonym.getTargetDatabase());
                ...
                MultiValuedMap<String, Synonym> tableMap = schemaMap.get(synonym.getTargetSchema());

                if (null == tableMap)
                {
                    tableMap = new ArrayListValuedHashMap<>();
    ...
    static class SynonymJdbcMetaDataLocator extends StandardJdbcMetaDataLocator
    {
        ...
        {
            super(targetScope, synonym.getTargetSchema(), SynonymTableResolver.escapeName(targetScope, synonym.getTargetSchema()), synonym.getTargetTable(), SynonymTableResolver.escapeName(targetScope, synonym.getTargetTable()));
    ...
    static class SynonymForeignKeyResolver extends StandardForeignKeyResolver
    {
        ...
        public ImportedKey getImportedKey(String fkName, String pkSchemaName, String pkTableName, String pkColumnName, String colName)
        {
            Map<String, MultiValuedMap<String, Synonym>> reverseMap = getReverseMap(_synonymScope, _targetScope);
            ...
            {
                MultiValuedMap<String, Synonym> mmap = reverseMap.get(pkSchemaName);
