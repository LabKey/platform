...
public class VisualizationController extends SpringActionController
{
    ...
    abstract class ExportSVGAction extends BaseViewAction<Object>
    {
        ...
        protected @NotNull SvgSource getSVGSource() throws Exception
        {
            if (null == _svgSource)
            {
                ...
                if (null != contentType && contentType.startsWith("text/xml"))
                    svgSource = PageFlowUtil.getReaderContentsAsString(request.getReader());
                else
                    svgSource = request.getParameter("svg");

                _svgSource = SvgSource.of(svgSource);
            }
    ...
    public class ExportPDFAction extends ExportSVGAction
    {
        ...
        public ModelAndView handleRequest() throws Exception
        {
            DocumentConversionService.get().svgToPdf(getSVGSource(), getFilename("pdf"), getViewContext().getResponse());
            return null;
        }
    }
