...
Ext4.define('LABKEY.ext4.GenericChartPanel', {
    ...
    {
        var aes, scales, plot, plotConfig, customRenderType, hasNoDataMsg, newChartDiv, response;
        ...
        response = LABKEY.vis.GenericChartHelper.doValueConversion(chartConfig, aes, this.renderType, this.chartData.rows);
        if (!Ext4.Object.isEmpty(response.processed))
        {
            //re-generate aes based on new converted values
            aes = LABKEY.vis.GenericChartHelper.generateAes(chartType, chartConfig.measures, this.chartData.schemaName, this.chartData.queryName);
            if (response.warningMessage) {
                this.addWarningText(response.warningMessage);
            }
        }
