...
public void nameNotNull() throws Exception
{
    try
    {
        createSampleType(null, List.of(new GWTPropertyDescriptor("name", "string")), null);
    }
...
public void reservedNameFirst() throws Exception
{
    try
    {
        createSampleType("First", List.of(new GWTPropertyDescriptor("name", "string")), null);
    }
    catch (ApiUsageException ee)
    {
...
public void reservedNameAll() throws Exception
{
    try
    {
        createSampleType("All", List.of(new GWTPropertyDescriptor("name", "string")), null);
    }
    catch (ApiUsageException ee)
    {
...
public void nameScale() throws Exception
{
    try
    {
        createSampleType(StringUtils.repeat("a", 1000), List.of(new GWTPropertyDescriptor("name", "string")), null);
    }
    catch (ApiUsageException ee)
    {
...
public void nameExpressionScale() throws Exception
{
    try
    {
        ...
        createSampleType(generateSampleTypeName(), props, StringUtils.repeat("a", 1000));
    }
    catch (ApiUsageException ee)
    {
...
public void idColsUnset_nameExpressionNull_noNameProperty() throws Exception
{
    try
    {
        ...
        createSampleType(generateSampleTypeName(), props, null);
...
public void idColsUnset_nameExpressionNull_hasNameProperty() throws Exception
{
    ...
    final ExpSampleType st = createSampleType(generateSampleTypeName(), props, null);
...
public void idColsUnset_nameExpression_hasNameProperty() throws Exception
{
    ...
    createSampleType(generateSampleTypeName(), props, "S-${prop}.${age}");
}
...
public void idColsSet_nameExpressionNull_noNameProperty() throws Exception
{
    ...
    final ExpSampleType st = SampleTypeService.get().createSampleType(c, _user,
            generateSampleTypeName(), null, props, Collections.emptyList(),
            0, 1, -1, -1, null, null);
...
public void idColsSet_nameExpressionNull_hasUnusedNameProperty() throws Exception
{
    try
    {
        ...
        SampleTypeService.get().createSampleType(c, _user,
                generateSampleTypeName(), null, props, Collections.emptyList(),
                1, -1, -1, -1, null, null);
        fail("Expected exception");
    }
...
public void idColsSet_nameExpressionNull_hasNameProperty() throws Exception
{
    ...
    insertSampleRows(st.getName(), List.of(CaseInsensitiveHashMap.of("name", expectedName1, "prop", "blue", "age", 10)));
    assertExpectedName(st, expectedName1);
    ...
    svc.insertRows(_user, c, rows, errors, null, null);
...
public void testNameExpression() throws Exception
{
    ...
    final ExpSampleType st = createSampleType(generateSampleTypeName(), props, "S-${prop}.${age}.${genId:number('000')}");
...
public void testBlankRows() throws Exception
{
    ...
    final ExpSampleType st = createSampleType(generateSampleTypeName(), props, "S-${now:date}-${dailySampleCount}");
    ...
    List<Map<String, Object>> rows = List.of(CaseInsensitiveHashMap.of(), CaseInsensitiveHashMap.of(), CaseInsensitiveHashMap.of());
    List<Map<String, Object>> inserted = insertSampleRows(st.getName(), rows);
    ...
    var insertedRows = insertSampleRows(st.getName(), dataMaps);
    ...
    QueryUpdateService svc = getSampleTypeUpdateService(st.getName());
    BatchValidationException errors = new BatchValidationException();
    svc.updateRows(_user, c, Collections.singletonList(updated), null, errors, null, null);
    ...
    // and a delete
    svc.deleteRows(_user, c, Collections.singletonList(updated), null, null);
    allSamples = st.getSamples(c);
...
public void testUpdateSomeParents() throws Exception
{
    ...
    List<Map<String, Object>> inserted = updateService.insertRows(_user, c, rows, errors, null, null);
    assertFalse(errors.hasErrors());
    ...
    inserted = updateService.insertRows(_user, c, rows, errors, null, null);
    assertFalse(errors.hasErrors());
    ...
    try
    {
        getSampleTypeUpdateService(parent2Type.getName()).updateRows(_user, c, rows, null, errors, null, null);
    ...
    // Attempt to merge using rowIds
    {
        ...
        updateService.mergeRows(_user, c, MapDataIterator.of(rows), errors, null, null);
    ...
    try
    {
        ...
        updateService.updateRows(_user, c, rows, null, errors, null, null);
        fail("Expected to throw exception");
    }
    catch (Exception e)
    {
    ...
    ExpLineage lineage = ExpLineageService.get().getLineage(c, _user, C1, opts);
    ...
    lineage = ExpLineageService.get().getLineage(c, _user, C4, opts);
    ...
    lineage = ExpLineageService.get().getLineage(c, _user, C2, opts);
    ...
    lineage = ExpLineageService.get().getLineage(c, _user, C5, opts);
...
public void testParentColAndDataInputDerivation() throws Exception
{
    ...
    final ExpSampleType st = createSampleType(generateSampleTypeName(), props, null);
    String sampleTypeName = st.getName();
    String materialInputsName = "MaterialInputs/" + sampleTypeName;
    ...
    rows.add(CaseInsensitiveHashMap.of("name", "C", "data", 12, "parent", null, materialInputsName, "B"));
    rows.add(CaseInsensitiveHashMap.of("name", "D", "data", 12, "parent", "B", materialInputsName, "C"));
    rows.add(CaseInsensitiveHashMap.of("name", "E", "data", 12, "parent", null, materialInputsName, "B,C"));
    ...
    List<Map<String, Object>> inserted = insertSampleRows(sampleTypeName, rows);
    ...
    assertNotNull(A);
    ExpLineage lineage = ExpLineageService.get().getLineage(c, _user, A, opts);
    ...
    assertEquals(0, A.getAliquotCount());
    assertNotNull(B);
    lineage = ExpLineageService.get().getLineage(c, _user, B, opts);
    ...
    assertEquals(0, A.getAliquotCount());
    assertNotNull(C);
    lineage = ExpLineageService.get().getLineage(c, _user, C, opts);
    assertEquals(1, lineage.getMaterials().size());
    ...
    assertEquals(0, A.getAliquotCount());
    assertNotNull(D);
    lineage = ExpLineageService.get().getLineage(c, _user, D, opts);
    ...
    assertNotNull(E);
    lineage = ExpLineageService.get().getLineage(c, _user, E, opts);
    ...
    lineage = ExperimentServiceImpl.get().getLineage(c, _user, Set.of(derivationRun), new ExpLineageOptions(true, false, 1));
    ...
    rows = new ArrayList<>();
    rows.add(CaseInsensitiveHashMap.of("rowId", D.getRowId(), materialInputsName, "B,E"));

    List<Map<String, Object>> updated = updateSampleRows(st.getName(), rows);
    assertEquals(1, updated.size());

    ExpMaterial D2 = st.getSample(c, "D");
    lineage = ExpLineageService.get().getLineage(c, _user, D2, opts);
    assertEquals(2, lineage.getMaterials().size());
    assertTrue("Expected 'D' to be derived from 'B'", lineage.getMaterials().contains(B));
    ...
    // Issue 43241: Display of dates from Input/Output columns for sample types does not use the project date format
    {
        ...
        var response = ViewServlet.GET(url, _user, null);
...
public void testSampleTypeWithVocabularyProperties() throws Exception
{
    ...
    Domain mockDomain = helper.createVocabularyTestDomain(_user, c);
...
{
    UserSchema auditSchema = AuditLogService.get().createSchema(_user, c);
    ...
    List<SampleTimelineAuditEvent> events = AuditLogService.get().getAuditEvents(c, _user, SampleTimelineAuditEvent.EVENT_TYPE, f, new Sort("-RowId"));
    ...
    // check audit log
    events = AuditLogService.get().getAuditEvents(c, _user, SampleTimelineAuditEvent.EVENT_TYPE, f, new Sort("-RowId"));
    assertFalse(events.isEmpty());
...
public void testExpMaterialPermissions() throws Exception
{
    ...
    var schema = QueryService.get().getUserSchema(_user, c, ExpSchema.SCHEMA_EXP);
    var materialsTable = schema.getTableOrThrow(ExpSchema.TableType.Materials.name());
    var qus = materialsTable.getUpdateService();
    assertNotNull(qus);
    assertTrue(qus.hasPermission(_user, ReadPermission.class));
    assertTrue(qus.hasPermission(_user, DeletePermission.class));
    assertFalse(qus.hasPermission(_user, InsertPermission.class));
    assertFalse(qus.hasPermission(_user, UpdatePermission.class));
    ...
    try
    {
        ExperimentServiceImpl.get().deleteMaterialByRowIds(
                _user, c, List.of(stSampleId, m.getRowId()), true, null, false, false);
        fail("Expected to throw exception");
    }
    catch (Exception e)
    {
    ...
    {
        ExperimentServiceImpl.get().deleteMaterialByRowIds(
                _user, c, List.of(stSampleId, m.getRowId()), true, st, false, false);
        fail("Expected to throw exception");
    }
    catch (Exception e)
    {
    ...
    // verify read via QUS
    var rows = qus.getRows(_user, c, List.of(CaseInsensitiveHashMap.of("rowId", m.getRowId())));
    ...
    rows = qus.deleteRows(_user, c, List.of(CaseInsensitiveHashMap.of("rowId", m.getRowId())), null, null);
...
public void testInsertOptionUpdate() throws Exception
{
    ...
    try
    {
        qus.updateRows(_user, c, rows, null, errors, null, null);
        fail("Expected to throw exception");
    }
    catch (Exception e)
    {
...
@Test
public void testSampleTypeFromTemplateReservedColumnNames() throws Exception
{
    ExpProvisionedTableTestHelper.requireSimpleTestModule(c);
    final String domainName = "SamplesFromTemplate";
    ...
    DomainTemplate template = templateGroup.getTemplate("Samples");
...
private static String generateSampleTypeName()
{
...
{
    return SampleTypeService.get().createSampleType(c, _user, sampleTypeName, null, props, emptyList(), -1, -1, -1, -1, nameExpression, null);
}

private @NotNull UserSchema getSampleSchema()
{
    UserSchema schema = QueryService.get().getUserSchema(_user, c, SamplesSchema.SCHEMA_SAMPLES);
...
private List<Map<String, Object>> insertSampleRows(String sampleType, List<Map<String, Object>> rows) throws Exception
{
    ...
    List<Map<String, Object>> ret = svc.insertRows(_user, c, rows, errors, null, null);
    if (errors.hasErrors())
        throw errors;
    return ret;
}
...
private int mergeSampleRows(String sampleType, List<Map<String, Object>> rows) throws Exception
{
    ...
    int count = svc.mergeRows(_user, c, MapDataIterator.of(rows), errors, config, null);
    if (errors.hasErrors())
        throw errors;
    return count;
}
...
private List<Map<String, Object>> updateSampleRows(String sampleType, List<Map<String, Object>> rows) throws Exception
{
    ...
    QueryUpdateService svc = getSampleTypeUpdateService(sampleType);
    Map<Enum, Object> config = Map.of(DetailedAuditLogDataIterator.AuditConfigs.AuditBehavior, AuditBehaviorType.DETAILED);
    List<Map<String, Object>> ret = svc.updateRows(_user, c, rows, null, errors, config, null);
    if (errors.hasErrors())
        throw errors;
    return ret;
}
