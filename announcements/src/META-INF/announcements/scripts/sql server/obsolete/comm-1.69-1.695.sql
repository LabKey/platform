...
-- For each thread, select RowId, EntityId, Container, Body, CreatedBy, and Created from the original post and add Title, Status,
...
CREATE VIEW comm.Threads AS
    SELECT y.RowId, y.EntityId, y.Container, y.Body, PropsId AS LatestId, props.Title, props.AssignedTo, props.Status, props.Expires, props.CreatedBy AS ResponseCreatedBy, props.Created AS ResponseCreated, y.CreatedBy, y.Created FROM
    (
