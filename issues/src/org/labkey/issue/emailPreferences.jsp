...
<form action="<%=h(buildURL(IssuesController.EmailPrefsAction.class))%>" method="post">
    ...
    Send me email when <%=indefArticle%> <%=h(names.singularName)%> is opened and assigned to me<br>
    <input type="checkbox" value="2" name="emailPreference" <%=(emailPrefs & IssueManager.NOTIFY_ASSIGNEDTO_UPDATE) != 0 ? " checked" : ""%>>
    Send me email when <%=indefArticle%> <%=h(names.singularName)%> that's assigned to me is modified<br>
    <input type="checkbox" value="4" name="emailPreference" <%=(emailPrefs & IssueManager.NOTIFY_CREATED_UPDATE) != 0 ? " checked" : ""%>>
    Send me email when <%=indefArticle%> <%=h(names.singularName)%> I opened is modified<br>
    <hr/>
    <input type="checkbox" value="8" name="emailPreference" <%=(emailPrefs & IssueManager.NOTIFY_SELF_SPAM) != 0 ? " checked" : ""%>>
    Send me email notifications when I enter/edit <%=indefArticle%> <%=h(names.singularName)%><br>
    <br>
