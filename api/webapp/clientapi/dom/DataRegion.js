...
(function($) {
                ...
                if (hasOffset) {
                    ...
                    var elems = [
                        '<div class="lk-menu-drop dropdown paging-widget">',
                            '<a data-toggle="dropdown" class="unselectable labkey-down-arrow dropdown-toggle labkey-paginationText">'+ _getPaginationText(this) + '</a>',
                            '<ul class="dropdown-menu dropdown-menu-left">',
                                '<li ' + (!showFirst ? 'aria-disabled="true" class="disabled"' : '') + '><a id="' + this.showFirstID + '" tabindex="0">Show first</a></li>',
                                '<li ' + (!showLast ? 'aria-disabled="true" class="disabled"' : '') + '><a id="' + this.showLastID + '" tabindex="0">Show last</a></li>',
                                '<li ' + (!showAll ? 'aria-disabled="true" class="disabled"' : '') + '><a id="' + this.showAllID + '" tabindex="0">Show all</a></li>',
    ...
    var _showFirstEnabled = function(region) {
        return region.offset && region.offset > 0;
    };

    var _showLastEnabled = function(region) {
        var low = region.offset + 1;
        var high = region.offset + region.rowCount;
        return !(low === 1 && high === region.totalRows) && (region.offset + region.maxRows <= region.totalRows);
    };
    ...
    var _getPaginationText = function(region) {
        var hasTotal = $.isNumeric(region.totalRows);
        var low = region.offset + 1;
        var high = region.offset + region.rowCount;

        var paginationText = low.toLocaleString() + ' - ' + high.toLocaleString();
        if (region.showPaginationCount || region.showPaginationCountAsync) {
            if (hasTotal) {
                paginationText += ' of ' + region.totalRows.toLocaleString();
            } else if (region.loadingTotalRows) {
                paginationText += ' of <i class="fa fa-spinner fa-pulse" ></i>';
            }
        }
