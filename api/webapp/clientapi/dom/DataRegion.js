...
(function($) {
    ...
    var _load = function(region, newParams, skipPrefixes, callback, scope) {
        ...
        if (region.showPaginationCountAsync) {
        ...
        // setTimeout(() => {
            LABKEY.Query.selectRows({
                ...
                success: function(result) {
                    region.totalRows = result.rowCount;
                    region.loadingTotalRows = false;
                    console.log('total rowCount', result.rowCount, _getPaginationText(region));
                    _getBarSelector(region).find('a.labkey-paginationText').html(_getPaginationText(region));
                },
                failure: function(result) {
                    // TODO handle failure case, console.error?
                    region.loadingTotalRows = false;
                    _getBarSelector(region).find('a.labkey-paginationText').html(_getPaginationText(region));
                }
            });
