...
LABKEY.DataRegion = Ext.extend(Ext.Component,
        /** @lends LABKEY.DataRegion.prototype */
        {
            constructor: function (config)
            {
                ...
                /**
                 * Config Options
                 *  domId      - DOM safe id for the DataRegion. This is generated by the renderer.
            ...
            {
                this.form = document.forms[this.domId + '-form'];
                this.table = Ext.get(this.domId);
                var msgEl = Ext.get(this.domId + '-msgbox');
                if (msgEl)
                {
                    this.msgbox = new LABKEY.MessageArea({parent: msgEl});
                    this.msgbox.on('rendermsg', this._onRenderMessageArea, this);
                }
                this.header = Ext.get(this.domId + '-header');
                this.footer = Ext.get(this.domId + '-footer');
            ...
            onSelectChange: function ()
            {
                ...
                LABKEY.Utils.signalWebDriverTest('dataRegionUpdate', this.selectedCount);
            },
            ...
            _hideButtonPanel: function(headerOrFooter, animate)
            {
                ...
                if (this.currentPanelId)
                {
                    ...
                    var callback = function()
                    {
                        ...
                        var button = Ext.query('#' + this.domId + ' .labkey-menu-button-active')[0];
                        if (button)
                        {
...
LABKEY.DataRegion.plugins.HeaderLock = (function ()
{
    ...
    return {
        init: function (dr)
        {
            ...
            dr.headerRow = Ext.get(dr.domId + '-header-row');
            if (!dr.headerRow)
            {
                dr._allowHeaderLock = false;
                return;
            ...
            dr.headerSpacer = Ext.get(dr.domId + '-header-row-spacer');
            dr.colHeaderRow = Ext.get(dr.domId + '-column-header-row');
            dr.colHeaderRowSpacer = Ext.get(dr.domId + '-column-header-row-spacer');
            dr.paginationEl = Ext.get(dr.domId + '-header');
