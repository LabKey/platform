...
Ext4.require([
    ...
    function(){
        if (typeof _accumulatorCfg != 'undefined') {
            Ext4.Perf.setup(_accumulatorCfg);
        }
        _syncStatusCheck();
    }
)

Ext4.define('PageAnalyzer.hook.Context', function () {
    ...
    function getLayoutResults (me, layout, reported, orphan) {
        ...
        Ext4.Object.each(layout.blockedBy, function (t) {
            results.blocks.push(t);
        });

        Ext4.Object.each(me.triggersByLayoutId[layout.id], function (name, info) {
            var item = info.item,
        ...
        for (i = 0; i < 2; ++i) {
            Ext4.Object.each(me.layouts, function (id, childLayout) {
    ...
    function beforeRun (me) {
        ...
        return {
            startTime: Ext4.perf.getTimestamp()
        };
    }
    ...
    function afterRun (me, before, ok) {
        ...
        var data = {
            duration: Ext4.perf.getTimestamp() - before.startTime,
            time: t2,
            success: ok,
        ...
        Ext4.Object.each(me.layouts, function (id, layout) {
        ...
        _layoutRuns.push(Ext4.encode(data));
    }
    ...
    return {
        ...
        flush: function() {
            var start = Ext4.perf.getTimestamp();
            this.callParent(arguments);
            this.flushTime += Ext4.perf.getTimestamp() - start;
            this.flushCount++;
        },
        ...
        flushInvalidates: function() {
            var start = Ext4.perf.getTimestamp();
            this.callParent(arguments);
            this.flushInvalidateTime += Ext4.perf.getTimestamp() - start;
        ...
        flushLayouts: function (queueName, methodName, dontClear) {
            ...
            this.callParent(arguments);
            stats.time += Ext4.perf.getTimestamp() - start;
            stats.count++;
        },
...
Ext4.define('PageAnalyzer.hook.ContextItem', function () {
    function getLayoutName (layout) {
        return layout.owner.id + '<' + layout.type + '>';
    }
        ...
        addBoxChild: function (boxChildItem) {
            ...
            if (boxChildren && boxChildren.length == 1) {
                ...
                boxParents = this.context.boxParents ||
                            (this.context.boxParents = new Ext4.util.MixedCollection());
                boxParents.add(this);
            }
