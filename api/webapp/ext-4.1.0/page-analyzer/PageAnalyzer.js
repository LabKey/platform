...
var path = LABKEY.contextPath + '/ext-4.1.0/page-analyzer/';
...
Ext.define('PageAnalyzer.MainForm', {
            ...
            tree = {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        width: 40,
                        items: [{
                            icon: LABKEY.contextPath + '/ext-4.1.0/page-analyzer/resources/images/delete.gif',
                            iconCls: 'pgan-delete-row',
                            tooltip: 'Delete this run',
                            handler: this.onDeleteLayoutRun,
                            scope: this
                        }, {
                            icon: LABKEY.contextPath + '/ext-4.1.0/page-analyzer/resources/images/info.gif',
    ...
    getTopLevelComponents: function () {
        var me = this,
            all = me.target.Ext4.ComponentManager.all.getArray(),
            top = [];
    ...
    onGatherStats: function() {
        var me = this,
            target = me.target,
            perf = target.Ext4.Perf,
            con = me.viewport.down('#perfcon');

        // Only gather page startup stats at each reload, not on each gather
        if (!me.statsGatherCount) {
            perf.get("Initialize").enter().leave(target.Ext4._afterReadytime - target.Ext4._beforeReadyTime);
            if (target.Ext4._endTime) {
                perf.get("Load").enter().leave(target.Ext4._endTime - target.Ext4._startTime);
                perf.get("WaitForReady").enter().leave(target.Ext4._readyTime - target.Ext4._endTime);
            } else {
                perf.get("BeforeReady").enter().leave(target.Ext4._readyTime - target.Ext4._startTime);
            }
        }
    ...
    getState: function() {
        ...
        if (target && target.Ext4 && target.Ext4.isReady) {

            if (target._layoutRuns && target.Ext4._readyTime) {
                return states.ready;
            }
            if (target._hooking) {
    ...
    injectScript: function (scriptSrc) {
        var me = this,
            url = scriptSrc, //me.getHrefMinusHash().replace(/\/[^/]+$/, '/' + scriptSrc),
    ...
    injectHooks: function() {
        ...
        me.injectScript(LABKEY.contextPath + '/ext-4.1.0/page-analyzer/hooks.js');
    },
    ...
    updateConnectedState: function() {
        ...
        if (state == states.loaded) {
            ...
            if (me.reloading) {
                ...
                me.reloading = false;
                me.target.Ext4._continueFireReady = true;
                me.target.Ext4.EventManager.readyEvent.fire();
            }
            me.updateLayoutRuns();
        }
    }
