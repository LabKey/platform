...
/*========================================================================
 * 
 * This section contains the Ext4.Compat object definition.
...
Ext4.Compat = function(){
    ...
    consoleOut = function(type, msg){
        ...
        if(Ext4.Array.indexOf(notificationCache, '__initMsg') == -1){
            ...
            if(!Ext4.Compat.showErrors){
                var errMsg = '#### Set "Ext4.Compat.showErrors = true;" in your code to include a stack trace with each warning.';
    ...
    return {
         ...
         * and also honors the Ext4.Compat.silent flag. Should be used for any breaking change
         ...
         * and also honors the Ext4.Compat.silent flag. Should be used for informational
        ...
        /**
         * This method should generally be eschewed in favor of Ext4.compat.deprecate. However it might be necessary
         ...
         * @param {String} pkg (optional) The package ("Core") or component ("Ext4.grid.Grid") being referenced (defaults to '')
...
(function(){
    ...
    Ext4.apply(Ext4, {
        ...
        // This is still aliased in Ext4.Array
//        toArray: function() {
//            deprecate({pkg:'Ext', member:'toArray', alt:'Ext.Array.toArray'});
//            return Ext4.Array.toArray.apply(Ext4.Array, arguments);
//        },
...
//        iterate: function() {
//            Ext4.each.apply(this, arguments);
//            deprecate({pkg:'Ext', member:'iterate', alt:'each',
//                msg:'Ext.each is now a wrapper for both Ext4.Array.forEach and Ext4.Object.each.'});
//        }
    });
        ...
        getterMsg: function(){
            deprecate({pkg:'Ext', member:'version', type:'property', alt:'getVersion (method)',
                msg:'Ext.getVersion() takes one argument which is the package name to look up. This will be defaulted '+
                    ...
                    'simple string representation use the Ext4.getVersion("extjs").version property.'});
        }
    });
...
//                msg:'Ext.getVersion() takes one argument which is the package name to look up. This will be defaulted '+
//                    'to "extjs" but could also be "core" or "touch". Note also that getVersion returns an object. For the '+
//                    'version detail representation use Ext4.getVersion("extjs") and inspect the properties major, minor and patch.'});
//        }
//    });
     ...
     * Ext4.lib.Dom
     *-------------------------------------------------------------*/
    // Helper function for aliasing Ext4.lib.Dom
     ...
     * Ext4.lib.Event
     *-------------------------------------------------------------*/
    // Helper function for aliasing Ext4.lib.Event
    ...
    /*-------------------------------------------------------------
     * Date overrides.
     * NOTE: The methods from the Date class under Ext4.util
...
//            notify('Ext.Element.child now returns a single direct child in 4.0 (switched with Ext4.Element.down). '+
//                'Reverting to the 3.x behavior that matches at any depth -- please verify this call when migrating to Ext 4.');
//
//            var n = Ext4.DomQuery.selectNode(selector, this.dom);
//            return returnDom ? n : Ext4.get(n);
//        },
//        down : function(selector, returnDom){
//            notify('Ext.Element.down now returns a single child at any depth in 4.0 (switched with Ext4.Element.child). '+
        ...
        animate: function(args, duration, onComplete, easing) {
            if (arguments.length > 1 || Ext4.isEmpty(args)) {
                deprecate({pkg:'Ext.Element', member:'animate', msg:'Ext.Element.animate should be called with a single configuration object. '+
                    'See the header API docs for Ext4.util.Animate for details on the supported config attribute syntax.'});
            }
            var me = this;
            if (!me.id) {
                me = Ext4.get(me.dom);
...
//    Ext4.extend = function(subclass, superclass, members) {
//        deprecate({pkg:'Ext', member:'extend', alt:'Ext.define',
//            msg:'Declare your class with Ext4.define() and use the "extend" config to specify the superclass.'});
//    
//        if (arguments.length === 2 && Ext4.isObject(superclass)) {
//            members = superclass;
//            superclass = subclass;
//            subclass = null;
//        }
...
//                subclass.constructor = Ext4.apply({}, subclass);
//            }
//            cls = new Ext4.Class(subclass, members);
//        }
//        else {
//            cls = new Ext4.Class(members);
//        }
//
