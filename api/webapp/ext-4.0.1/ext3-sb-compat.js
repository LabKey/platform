 ...
 * Ext4.Compat is defined in ext3-core-compat.js.
 *
 *========================================================================*/
...
(function(){
    ...
    /*-------------------------------------------------------------
     * Date (from the Ext4.util folder in 3.x)
     *-------------------------------------------------------------*/
    (function(){
    ...
    if (Ext4.ComponentManager) {
        var regMsg = 'Calling a separate function to register custom types is no longer necessary. '+
                     'Switch your class definition to use Ext4.define with "{0}" as the alias config.';
        ...
        var checkClassRef = function(pkg, cls, member) {
            if (!Ext4.isString(cls)) {
                breaking({pkg:pkg, member:member, msg:'You must either convert the passed class reference to a string -- e.g. ' + 
                    pkg + '.' + member + '("myclass", "Ext4.ux.MyClass") -- or change your class definition to use Ext4.define. '+
            ...
            constructor: Ext4.Function.createInterceptor(Ext4.toolbar.Paging.prototype.constructor, function(config) {
                if (config.paramNames) {
                    var msg = 'Instead of params specific to this toolbar you should set any needed options on the associated store/proxy. '+
                        'See the header docs for Ext4.data.Store for details. The defaults for PagingToolbar {start: \'start\', limit: \'limit\'} '+
    ...
    if (Ext4.grid.Panel) {
        ...
        Ext4.grid.EditorGridPanel = function(config) {
            deprecate({pkg:'Ext.grid.EditorGridPanel', msg:'EditorGridPanel no longer exists as a separate class. Instead just '+
                'create a standard GridPanel and include the CellEditing plugin, e.g. "plugins: Ext4.create("Ext4.grid.plugin.CellEditing", {...})'});
    ...
    if (Ext4.grid.View) {
                        ...
                        if (rowParams.body) {
                            ...
                            breaking({pkg:'Ext.grid.View', member:'getRowClass.rowParams.body', single:true,
                                msg:'To implement a custom row body you must add the RowBody feature (ftype:"rowbody") '+
                                    ...
                                    '(or use the Ext4.grid.RowBodyPlugin helper class). Unfortunately this cannot be '+
                        ...
                        if (rowParams.bodyStyle) {
                            ...
                            deprecate({pkg:'Ext.grid.View', member:'getRowClass.rowParams.bodyStyle', single:true,
                                msg:'To implement a custom row styles you must add the RowBody feature (ftype:"rowbody") '+
                                    'to the grid\'s viewConfig and override the "getAdditionalData" template method '+
                                    '(or use the Ext4.grid.RowBodyPlugin helper class). Note that in 3.x this property '+
                        ...
                        if (rowParams.tstyle) {
                            ...
                            deprecate({pkg:'Ext.grid.View', member:'getRowClass.rowParams.tstyle', single:true,
                                msg:'Grid row bodies no longer use a wrapping TABLE element, so the "tstyle" property '+
                                    ...
                                    'the "getAdditionalData" template method (or use the Ext4.grid.RowBodyPlugin helper class). '+
        ...
        Ext4.grid.GroupingView = function(config) {
            breaking({pkg:'Ext.grid.GroupingView', msg:'GroupingView no longer exists as a separate class, and grid views should '+
                ...
                'e.g. "features: Ext4.create("Ext4.grid.feature.Grouping", {...}). Unfortunately there is no way to alias a call to this '+
                ...
                if (url) {
                    ...
                    deprecate({pkg:'Ext.tree.TreePanel', member:'loader', type:'config', alt:'TreeStore',
                        msg:'The TreeLoader class and TreePanel.loader config have been removed. Trees now use the TreeStore '+
                            'which provides all standard Ext4.data.Store loading capabilities.'});
                }
...
//                    sortCollection = Ext4.create('Ext.util.MixedCollection');
    ...
    if (Ext4.form.field.Base) {
        Ext4.apply(Ext4.form.field.Base.prototype, {
            initComponent: Ext4.Function.createInterceptor(Ext4.form.field.Base.prototype.initComponent, function() {
                // Many legacy examples modify the default msgTarget on the Ext4.form.Field class's prototype; this doesn't
                // work anymore since Field is a mixin. Copy to Ext4.form.field.Base and inform about change and the new
                ...
                if (msgTarget && msgTarget !== 'qtip') {
                    deprecate({pkg:'Ext.form.Field', member:'msgTarget', type:'config', single: true,
                        msg:'Found an overridden value for Ext4.form.Field.prototype.msgTarget -- Ext4.form.Field is ' +
                            'now Ext4.form.field.Base; either override msgTarget on Ext4.form.field.Base\'s prototype ' +
                            'or use the new recommended Ext4.form.Panel#fieldDefaults object instead.'});
        ...
        Ext4.apply(Ext4.form.field.Checkbox.prototype, {
            initComponent: Ext4.Function.createInterceptor(Ext4.form.field.Checkbox.prototype.initComponent, function() {
                this.addEvents(
                     ...
                     * @param {Ext4.form.field.Checkbox} this This checkbox
            ...
            initComponent: Ext4.Function.createInterceptor(Ext4.slider.Multi.prototype.initComponent, function() {
                if (this.plugins) {
                    Ext4.each(this.plugins, function(p){
                        if (p.getText) {
                            deprecate({pkg:'Ext.Slider', msg:'In 3.x the Ext4.slider.Tip plugin was required to provide custom slider tip text. '+
                ...
                if (config.url) {
                    ...
                    if (!config.proxy) {
                        deprecate({pkg:'Ext.data.Store', msg:'A store url was specified with no proxy config. Implcitily creating an AjaxProxy with that url. '+
                            'Please see the header docs for Ext4.data.Store for details on properly setting up your data components.'});
...
//                            'Please see the header docs for Ext4.data.Store for details on properly setting up your data components.'});
                        ...
                        if (config.reader.model) {
                            // the compat warning for this case is displayed below in the Ext4.data.Reader override where
                            ...
                            breaking({pkg:'Ext.data.Store', msg:'No valid model or field configuration could be found '+
                                'so this store could not be constructed. Please see the header docs for Ext4.data.Store for '+
            ...
            setDefaultSort : function(field, dir) {
                deprecate({pkg:'Ext.data.Store', member:'setDefaultSort', alt:'sorters (config)',
                    msg:'Either add the default sort via the "sorters" config or by adding it to the "sorters" property after the store is created. '+
                        'See the Ext4.data.Store header docs for details on configuring sorters.'});
    ...
    Ext4.data.GroupingStore = function(config) {
        deprecate({pkg:'Ext.data.GroupingStore', msg:'GroupingStore no longer exists as a separate class. Instead just '+
            'create a standard GridPanel and include the Grouping feature, e.g. "features: Ext4.create("Ext4.grid.feature.Grouping", {...})'});
    ...
    if (Ext4.data.Record) {
        Ext4.data.Record.create = function(o){
            deprecate({pkg:'Ext.data.Record', member:'create', msg:'There is no longer any need to statically define records. '+
                'You can simply define a new Model configured with the necessary fields via Ext4.define, extending Ext4.data.Model.'});
