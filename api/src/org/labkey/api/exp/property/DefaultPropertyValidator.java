...
public abstract class DefaultPropertyValidator implements ValidatorKind
{
    ...
    protected void createErrorMessage(IPropertyValidator validator, ColumnRenderProperties field, Object value, List<ValidationError> errors)
    {
    ...
    {
        if (Strings.CS.equals("~eq", type))
            return comparison == 0;
        else if (Strings.CS.equals("~neq", type))
            return comparison != 0;
        else if (Strings.CS.equals("~lte", type))
            return comparison != 1;
        else if (Strings.CS.equals("~gte", type))
            return comparison != -1;
        else if (Strings.CS.equals("~gt", type))
            return comparison == 1;
        else if (Strings.CS.equals("~lt", type))
            return comparison == -1;
        return false;
    }
}