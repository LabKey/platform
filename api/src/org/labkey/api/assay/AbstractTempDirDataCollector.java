...
public abstract class AbstractTempDirDataCollector<ContextType extends AssayRunUploadContext<? extends AssayProvider>> extends AbstractAssayDataCollector<ContextType>
{
    ...
    private void removeTempDir(ContextType context) throws ExperimentException
    {
        ...
        try
        {
            FileObject tempDir = ensureSubdirectory(context.getContainer(), TEMP_DIR_NAME);
            FileObject uploadAttemptDir = tempDir.resolveFile(uploadAttemptID);
            if (NetworkDrive.exists(uploadAttemptDir))
            {
                FileUtils.deleteDirectory(uploadAttemptDir.getPath().toFile());
            }
        }
            catch (IOException e)
        {
    ...
    private void removeFiles(ContextType context, Collection<Map<String, File>> assayFiles) throws ExperimentException
    {
        for (Map<String, File> fileMap : assayFiles)
        {
            for (File file : fileMap.values())
            {
                try
                {
                    FileObject assayFile = ensureUploadDirectory(context.getContainer(), DIR_NAME).resolveFile(file.getName());
                    FileUtils.deleteQuietly(assayFile.getPath().toFile());
                }
    ...
    public void initDir(ContextType context) throws ExperimentException
    {
        ...
        if (!context.getUploadedData().isEmpty())
        {
            ...
            try
            {
                ...
                FileObject tempDir = AssayFileWriter.ensureSubdirectory(context.getContainer(), TEMP_DIR_NAME);
                FileObject uploadAttemptDir = tempDir.resolveFile(uploadAttemptID);
                if (NetworkDrive.exists(uploadAttemptDir))
                {
                    ...
                    try
                    {
                        fileList = uploadAttemptDir.getChildren();
                    }
                    catch (FileSystemException fse)
                    {
                        // pass
                    ...
                    if (null != fileList)
                    {
                        for (FileObject file : fileList)
                        {
                            ...
                            for (Map.Entry<String, FileObject> entry : context.getUploadedData().entrySet())
                            {
                            ...
                            if (!save)
                            {
                                FileUtils.deleteQuietly(file.getPath().toFile());
                            }
                        }
                    }
                }
    ...
    protected FileObject getFileTargetDir(ContextType context) throws ExperimentException
    {
        if (context instanceof AssayRunUploadForm)
        {
            FileObject uploadAttemptDir = null;

            try
            {
                FileObject tempDir = ensureSubdirectory(context.getContainer(), TEMP_DIR_NAME);
                uploadAttemptDir = tempDir.resolveFile(((AssayRunUploadForm)context).getUploadAttemptID());

                if (!NetworkDrive.exists(uploadAttemptDir))
                {
                    FileUtil.mkdirs(uploadAttemptDir);
                }
                if (!uploadAttemptDir.isFolder())
                {
    ...
    @Nullable
    protected FileObject getFilePath(ContextType context, @Nullable ExpRun run, FileObject tempDirFile) throws ExperimentException
    {
        FileObject assayDir = ensureUploadDirectory(context.getContainer());
        return findUniqueFileName(tempDirFile.getName().getBaseName(), assayDir);
    }
    ...
    public Map<String, FileObject> uploadComplete(ContextType context, @Nullable ExpRun run) throws ExperimentException
    {
        ...
        Map<String, FileObject> uploadedData = context.getUploadedData();
        Map<String, FileObject> result = new HashMap<>(uploadedData);
        for (Map.Entry<String, FileObject> entry : uploadedData.entrySet())
        {
            fileToName.put(entry.getValue().getPath().toFile(), entry.getKey());
        }
        ...
        try
        {
            ...
            FileObject assayDir = ensureUploadDirectory(context.getContainer());
            FileObject tempDir = getFileTargetDir(context);
            for (FileObject tempDirFile : tempDir.getChildren())
            {
                FileObject assayDirFile = getFilePath(context, run, tempDirFile);
                if (assayDirFile != null)
                {
                    ...
                    for (ExpData expData : allData)
                    {
                        ...
                        {
                            expData.setDataFileURI(assayDirFile.getURI());
                    ...
                    if (run != null)
                    {
                        ...
                        run.setFilePathRoot(assayDir.getPath().toFile());
                    ...
                    handleTempFile(tempDirFile.getPath().toFile(), assayDirFile.getPath().toFile());
                }
                else
                {
            ...
            FileUtils.deleteDirectory(tempDir.getPath().toFile());
        }
    ...
    protected String getPreferredAssayId(FileObject primaryFile)
    {
        return primaryFile.getName().getBaseName();
    }
