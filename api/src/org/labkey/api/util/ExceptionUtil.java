...
public class ExceptionUtil
{
     ...
     * Key is the exception's hash as calculated by {@link #hashStackTrace(ExceptionStackTrace)}.
     */
    ...
    public static String logExceptionToMothership(@Nullable HttpServletRequest request, Throwable ex, boolean writeToLog4J)
    {
        ...
        for (Throwable t = ex ; t != null ; t = t.getCause())
        {
            ...
            if (t instanceof RuntimeSQLException runtimeSQLException)
            {
                // Unwrap RuntimeSQLExceptions
                t = runtimeSQLException.getSQLException();
            }
    ...
    @Nullable
    public static MothershipReport createReportFromThrowable(
    ...
    )
    {
        ...
        if (ex instanceof ServletException servletException && servletException.getRootCause() != null)
        {
            ...
            servletException.getRootCause().printStackTrace(printWriter);
        }
        String browser = request == null ? null : request.getHeader("User-Agent");

        ExceptionStackTrace stackTrace = new ExceptionStackTrace(stringWriter.getBuffer().toString(), false);

        if (!shouldSend(level, target.isLocal(), stackTrace))
            return null;

        if (extraInfo != null)
            stackTrace = new ExceptionStackTrace(stackTrace.stackTrace + "\n" + extraInfo, false);
    ...
    private static void prepareClientStackTrace(StringBuilder sb, BufferedReader reader) throws IOException
    {
        ...
        // matches against these to coalesce the same error generated by different applications.
        Pattern p = Pattern.compile("webpack://[\\w-]+/");
    ...
    private static String hashStackTrace(ExceptionStackTrace stackTrace)
    {
        return hashStackTrace(stackTrace.stackTrace, stackTrace.clientException);
    }

    public static String hashStackTrace(String fullStackTrace, boolean isClientException)
    {
        ...
        try
        {
            if (isClientException)
                prepareClientStackTrace(sb, reader);
            else
                prepareServerStackTrace(sb, reader);
        }
        catch (IOException e)
        {
    ...
    private record ExceptionStackTrace(String stackTrace, boolean clientException) {}
    ...
    {
        String errorCode = null;
        ExceptionStackTrace stackTrace = new ExceptionStackTrace(fullStackTrace, true);
    ...
    @NotNull
    private static MothershipReport createReportFromStacktrace(
    ...
    )
    {
        try
        {
            ...
            report.addParam("stackTrace", exceptionStackTrace.stackTrace);
            report.addParam("clientException", exceptionStackTrace.clientException);
