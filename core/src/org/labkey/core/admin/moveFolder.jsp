...
<table class="dataRegion">
    ...
    <%
        ...
        ActionURL moveURL = AdminControllerSpring.getMoveFolderURL(c, form.isAddAlias());    // Root is placeholder -- will get replaced by container tree
        AdminControllerSpring.MoveContainerTree ct = new AdminControllerSpring.MoveContainerTree(showAll ? "/" : form.getProjectName(), HttpView.currentContext().getUser(), ACL.PERM_ADMIN, moveURL);
    ...
    <td><a href="<%=h(AdminControllerSpring.getManageFoldersURL(c))%>"><img border=0 src='<%=PageFlowUtil.buttonSrc("Cancel")%>'></a></td><%
    if (form.isShowAll())
    {
        if (!c.isProject())
        {
            %><td><a href="<%=h(AdminControllerSpring.getShowMoveFolderTreeURL(c, form.isAddAlias(), false))%>"><img border=0 src='<%=PageFlowUtil.buttonSrc("Show Current Project Only")%>'></a></td><%
        }
    }
    else
    {
        %><td><a href="<%=h(AdminControllerSpring.getShowMoveFolderTreeURL(c, form.isAddAlias(), true))%>"><img border=0 src='<%=PageFlowUtil.buttonSrc("Show All Projects")%>'></a></td><%
    }
    %></tr>
</table>