        ...
        public void write(Container c, FolderExportContext ctx, VirtualFile vf) throws Exception
        {
            ...
            if (!groups.isEmpty())
            {
                ...
                for (Group group : groups)
                {
                    ...
                    GroupManager.exportGroupMembers(group, new ArrayList<>(SecurityManager.getGroupMembers(group, MemberType.GROUPS)), new ArrayList<>(SecurityManager.getGroupMembers(group, MemberType.ACTIVE_USERS)), xmlGroup);
                }
            }
        }
