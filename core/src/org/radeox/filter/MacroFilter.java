...
public class MacroFilter extends RegexTokenFilter {
  private static final Logger log = LogManager.getLogger(MacroFilter.class);
      ...
      if (!command.startsWith("$")) {
        ...
        try {
          ...
          buffer.append("<div class=\"error\">" + PageFlowUtil.filter(command + ": " + e.getMessage()) + "</div>");
            ...
            log.warn("MacroFilter: unable to format macro: {} in {}", result.group(1), context.getRenderContext(), e);
          buffer.append("<div class=\"error\">" + PageFlowUtil.filter(command + ": " + e.getMessage()) + "</div>");
          return;
        }
      } else {
