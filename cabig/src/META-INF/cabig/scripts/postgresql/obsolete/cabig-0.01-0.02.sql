...
-- Temporary: this table will be dropped and a bit added to core.Containers once
-- the trunk is open to core schema changes
CREATE TABLE cabig.temp_containers
(
	entityid ENTITYID NOT NULL UNIQUE
);


-- Temporary version of this view -- will change when we drop temp_containers table
CREATE VIEW cabig.Containers AS
    SELECT c.RowId, c.EntityId, c.Name, p.RowId AS ParentId FROM core.Containers c
        INNER JOIN cabig.temp_containers tc ON tc.EntityId = c.EntityId
        INNER JOIN core.Containers p ON c.Parent = p.EntityId;


create or replace view cabig.propertyvalue
as
...
create or replace view cabig.propertydescriptor
as
...
create or replace view cabig.domaindescriptor
as
...
create or replace view cabig.propertydomain
as
SELECT pd.domainid , pd.propertyid
FROM exp.propertydomain pd;


create or replace view cabig.customproperties
as
...
create or replace view cabig.protocoldefinition as
...
create or replace view cabig.protocolaction as
...
create or replace view cabig.runprotocol as
...
create or replace view cabig.protocolstep as
...
create or replace view cabig.actionsuccessor as
...
create or replace view cabig.successorpredecessor as
...
create or replace view cabig.data
as
...
create or replace view cabig.datainput
as
SELECT dataid, targetapplicationid, propertyid
FROM exp.datainput;


create or replace view cabig.experiment
as
...
create or replace view cabig.material
as
...
create or replace view cabig.materialinput
as
...
create or replace view cabig.protocolapplication
as
...
create or replace view cabig.protocolapplicationparameter
as
...
create or replace view cabig.protocolparameter
as
...
create or replace view cabig.runlist
as
SELECT experimentid, experimentrunid
FROM exp.runlist;


create or replace view cabig.dataruninputs as
...
create or replace view cabig.datarunoutputs as
SELECT di.dataid, di.propertyid, pa.runid
FROM exp.data d
	INNER JOIN exp.datainput di ON (d.rowid = di.dataid)
	INNER JOIN exp.protocolapplication pa ON (di.targetapplicationid = pa.rowid)
...
create or replace view cabig.materialruninputs as
...
create or replace view cabig.materialrunoutputs as
SELECT mi.materialid, mi.propertyid, pa.runid
FROM exp.material m
	INNER JOIN exp.materialinput mi ON (m.rowid = mi.materialid)
	INNER JOIN exp.protocolapplication pa ON (mi.targetapplicationid = pa.rowid)
