 ...
 */
public class StudyProductAntigenTable extends DefaultStudyDesignTable<StudyDesignQuerySchema>
{
    ...
    public static StudyProductAntigenTable create(Domain domain, StudyDesignQuerySchema schema, @Nullable ContainerFilter filter)
    {
    ...
    private StudyProductAntigenTable(Domain domain, TableInfo storageTableInfo, StudyDesignQuerySchema schema, @Nullable ContainerFilter containerFilter)
    {
        super(domain, storageTableInfo, schema, containerFilter);

        setName(StudyDesignQuerySchema.PRODUCT_ANTIGEN_TABLE_NAME);
    ...
    protected void initColumn(MutableColumnInfo col)
    {
        if ("ProductId".equalsIgnoreCase(col.getName()))
        {
            col.setFk(new LookupForeignKey("RowId")
            {
                ...
                {
                    return QueryService.get().getUserSchema(_userSchema.getUser(), _userSchema.getContainer(), StudyDesignSchema.STUDY_SCHEMA_NAME).getTable(StudyDesignQuerySchema.PRODUCT_TABLE_NAME);
                }
            });
        }
        else if ("Gene".equalsIgnoreCase(col.getName()))
        {
            col.setFk(new LookupForeignKey("Name")
            {
                ...
                public TableInfo getLookupTableInfo()
                {
                    return QueryService.get().getUserSchema(_userSchema.getUser(), _userSchema.getContainer(), StudyDesignSchema.STUDY_SCHEMA_NAME).getTable(StudyDesignQuerySchema.STUDY_DESIGN_GENES_TABLE_NAME);
                }
            });
        }
        else if ("SubType".equalsIgnoreCase(col.getName()))
        {
            col.setFk(new LookupForeignKey("Name")
            {
                ...
                public TableInfo getLookupTableInfo()
                {
                    return QueryService.get().getUserSchema(_userSchema.getUser(), _userSchema.getContainer(), StudyDesignSchema.STUDY_SCHEMA_NAME).getTable(StudyDesignQuerySchema.STUDY_DESIGN_SUB_TYPES_TABLE_NAME);
                }
            });
        }
    }
