...
public class StudyTreatmentProductTable extends DefaultStudyDesignTable
{
    ...
    public static StudyTreatmentProductTable create(Domain domain, StudyDesignQuerySchema schema, @Nullable ContainerFilter filter)
    {
    ...
    private StudyTreatmentProductTable(Domain domain, TableInfo storageTableInfo, StudyDesignQuerySchema schema, @Nullable ContainerFilter filter)
    {
         super(domain, storageTableInfo, schema, filter);

        setName(StudyDesignQuerySchema.TREATMENT_PRODUCT_MAP_TABLE_NAME);
    ...
    protected void initColumn(MutableColumnInfo col)
    {
        ...
        else if ("TreatmentId".equalsIgnoreCase(col.getName()))
        {
            col.setFk(new LookupForeignKey("RowId")
            {
                ...
                public TableInfo getLookupTableInfo()
                {
                    return QueryService.get().getUserSchema(_userSchema.getUser(), _userSchema.getContainer(), StudyDesignSchema.STUDY_SCHEMA_NAME).getTable(StudyDesignQuerySchema.TREATMENT_TABLE_NAME);
                }
            });
        }
        else if ("Route".equalsIgnoreCase(col.getName()))
        {
            col.setFk(new LookupForeignKey("Name")
            {
                ...
                public TableInfo getLookupTableInfo()
                {
                    return QueryService.get().getUserSchema(_userSchema.getUser(), _userSchema.getContainer(), StudyDesignSchema.STUDY_SCHEMA_NAME).getTable(StudyDesignQuerySchema.STUDY_DESIGN_ROUTES_TABLE_NAME);
                }
            });
        }
    }
