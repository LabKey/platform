 ...
 */
public class StudyProductTable extends DefaultStudyDesignTable<StudyDesignQuerySchema>
{
    ...
    public static StudyProductTable create(Domain domain, StudyDesignQuerySchema schema, @Nullable ContainerFilter containerFilter)
    {
    ...
    private StudyProductTable(Domain domain, TableInfo storageTableInfo, StudyDesignQuerySchema schema, @Nullable ContainerFilter containerFilter)
    {
         super(domain, storageTableInfo, schema, containerFilter);

        setName(StudyDesignQuerySchema.PRODUCT_TABLE_NAME);
    ...
    protected void initColumn(MutableColumnInfo col)
    {
        if ("Type".equalsIgnoreCase(col.getName()))
        {
            col.setFk(new LookupForeignKey("Name")
            {
                ...
                {
                    return QueryService.get().getUserSchema(_userSchema.getUser(), _userSchema.getContainer(), StudyDesignSchema.STUDY_SCHEMA_NAME).getTable(StudyDesignQuerySchema.STUDY_DESIGN_IMMUNOGEN_TYPES_TABLE_NAME);
                }
            });
        }
    }
