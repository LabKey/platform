 ...
 * Time: 12:11 PM
 */
public class SpecimenRefreshMaintenanceTask implements MaintenanceTask
{
    @Override
    ...
    public void run(Logger log)
    {
        ...
        {
            Study sourceStudy = getStudy(snapshot.getSource());

            if (null == sourceStudy)
                continue;

            Study destinationStudy = getStudy(snapshot.getDestination());
            ...
            Date sourceLastSpecimenLoad = svc.getLastSpecimenLoad(sourceStudy);
            Date destLastSpecimenLoad = svc.getLastSpecimenLoad(destinationStudy);

            // If null, specimens definitely haven't been updated since the snapshot
            if (null == sourceLastSpecimenLoad)
                continue;

            if (null == destLastSpecimenLoad ||
                    sourceLastSpecimenLoad.compareTo(destLastSpecimenLoad) > 0)
            {
                try
                {
    ...
    // Return Study if containerId is not null, container exists, and container has a study
    private @Nullable Study getStudy(GUID containerId)
    {
        if (null != containerId)
        {
